...
class Login(models.Model):
    ...
    def clean(self):
        ...
        if self.opcao == "Func":
            ...
            try:
                cadastro = CadastroVoluntario.objects.get(situacao='Funcionario', email=self.email, senha=self.senha)
            except CadastroVoluntario.DoesNotExist:
                # Se não encontrar, levanta uma exceção de validação
                raise ValidationError("Cadastro de Funcionário não encontrado ou e-mail/senha incorretos.")
        else:
...
class Admin(models.Model):
    ...
    email = models.EmailField(verbose_name="E-mail", max_length=255, unique=True, help_text="Digite o E-mail aqui.",
                              error_messages={'blank': 'Este campo não pode ficar em branco.',
                                              'invalid': 'O valor fornecido é inválido, verifique se tem @.'})
    senha = models.CharField(max_length=100)
    telefone = models.CharField(
        max_length=15, null=False, blank=True, verbose_name="Telefone",
        help_text="Digite seu Telefone aqui. (21) 90000-0000"
    )
    cpf = models.CharField(max_length=14, unique=True, null=False, blank=True, verbose_name="CPF",
                           help_text="Digite seu CPF aqui modelo: 000.000.000-00")
    situacao = models.CharField(max_length=100, verbose_name="Situação", default="Padrão")

    def mark_has_complete(self):
        if self.situacao:
            self.situacao = "Superior"
            self.save()
...
class CadastroVoluntario(models.Model):
    ...
    situacao = models.CharField(max_length=100, verbose_name="Situação", default="Candidato")
...
class Cadastro(models.Model):
    ...
    DExp = models.CharField(max_length=20, verbose_name="Data expiração", null=False,
    ...
    cozinha = models.IntegerField(verbose_name="Quantidade de cozinha(s)")
